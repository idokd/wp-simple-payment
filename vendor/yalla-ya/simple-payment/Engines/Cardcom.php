...
class Cardcom extends Engine {
  ...
  public function verify( $transaction ) {
    ...
    switch( intval( $operation ) ) {
      case 1:
        $code = $response[ 'InternalDealNumber' ];
...
//https://asmachta.co.il/?engine=Cardcom&payment_id=127046&redirect_url=https%3A%2F%2Fasmachta.co.il%2F%25d7%25a0%25d7%25a1%25d7%2597-%25d7%2598%25d7%2590%25d7%2591%25d7%2595%2F%3Fgf_simplepayment_return%3DaWRzPTJ8MTM0Nzk0Jmhhc2g9YWRkYmFjYmZiZGRjNDdlMTlkY2E1NzBiYjA0NmZkNzk&terminalnumber=125527&lowprofilecode=83fccd5d-6aed-4235-a1ab-62aceb769fad&ResponeCode=0&Operation=1&ResponseCode=0&Status=0
  // Sanitize url for Cardcom, so we wont send cardcom parameters
  public function url( $type, $params = null ) {
    if ( $url = parent::url( $type, $params ) ) {
      $parts = parse_url( $url );
      if ( isset( $parts[ 'query' ] ) ) {
        parse_str( $parts[ 'query' ], $query );
        foreach ( [ 'terminalnumber', 'lowprofilecode', 'ResponeCode', 'Operation', 'ResponseCode', 'Status', 'DealRespone', 'InvoiceResponseCode', 'OperationResponse', 'OperationResponseText', 'ReturnValue' ] as $param ) {
          unset( $query[ $param ] );
        }
