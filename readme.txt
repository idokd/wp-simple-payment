...
* New experimental feature "WooCommerce Fraud Detection": blocks the checkout (hides payment methods and shows a custom message) after repeated failed orders that share the same identity within a timeframe, with a cooldown, role whitelist and an option to exempt registered users. The identity is configurable per field - match by email, billing phone, IP address and/or user agent (order attribution). By default it links failed orders that share any matched value into a single cluster (e.g. X/Y, then X/Z, then B/Y count as 3); defaults to matching Email + Phone + IP over a 24 hour window (all configurable)
