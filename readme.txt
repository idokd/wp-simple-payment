...
* WooCommerce companion: the receiving store selects the gateway for incoming orders (the source site does not need to know the remote gateways); unknown incoming payment methods are cleared so the customer can choose on the payment page
